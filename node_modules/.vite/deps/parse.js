...
import { t as require_events } from "./events.js";
	...
	*/
	var ParseFile = class ParseFile {
		...
		*/
		constructor(name, data, type, metadata, tags) {
			...
			this._tags = tags || {};
			if (data !== void 0) if (typeof Buffer !== "undefined" && Buffer.isBuffer(data)) this._source = {
				format: "buffer",
				buffer: data,
				type: specifiedType
			};
		...
		*/
		save(options) {
			...
			if (!this._previousSave) if (this._source.format === "buffer" || this._source.format === "stream") {
	...
	var DefaultController = {
		...
		saveBinary: async function(name, source) {
			...
			if (useMasterKey) if (_CoreManager.default.get("MASTER_KEY")) {
	...
	exports.default = class ParseACL {
		...
		*/
		constructor(arg1) {
			...
			if (arg1 && typeof arg1 === "object") if (arg1 instanceof _CoreManager.default.getParseUser()) {
...
var require_promiseUtils = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function when(promises) {
		...
		const resolveOne = function() {
			total--;
			if (total <= 0) if (hadError) promise.reject(errors);
	...
	var RelationOp = class RelationOp extends Op {
		...
		applyTo(value, parent, key) {
			...
			if (value instanceof _ParseRelation.default) {
				if (this._targetClassName) if (value.targetClassName) {
...
var require_ObjectStateMutations = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function estimateAttribute(serverData, pendingOps, object, attr) {
		...
		for (let i = 0; i < pendingOps.length; i++) if (pendingOps[i][attr]) if (pendingOps[i][attr] instanceof _ParseOp.RelationOp) {
	...
	var ParseObject = class ParseObject {
		...
		*/
		...
		_getSaveParams() {
			...
			if (_CoreManager.default.get("ALLOW_CUSTOM_OBJECT_ID")) if (!this.createdAt) {
				method = "POST";
		...
		_migrateId(serverId) {
			if (this._localId && serverId) if (singleInstance) {
				const stateController = _CoreManager.default.getObjectStateController();
		...
		*/
		static extend(className, protoProps, classProps) {
			if (typeof className !== "string") if (className && typeof className.className === "string") return ParseObject.extend(className.className, className, protoProps);
	...
	var ParseQuery = class ParseQuery {
		...
		*/
		constructor(objectClass) {
			...
			*/
			...
			if (typeof objectClass === "string") if (objectClass === "User" && _CoreManager.default.get("PERFORM_USER_REWRITE")) this.className = "_User";
		...
		*/
		...
		async _handleOfflineQuery(params) {
			...
			if (params.skip) if (params.skip >= results.length) results = [];
		...
		*/
		static or() {
			...
			const query = new ParseQuery(_getClassNameFromQueries(queries));
		...
		async ajax(method, url, data, headers, options) {
			...
			const dispatch = async function() {
				...
				try {
					...
					} else if (status >= 500 || status === 0) if (!(typeof ReadableStream !== "undefined" && data instanceof ReadableStream) && ++attempts < _CoreManager.default.get("REQUEST_ATTEMPT_LIMIT")) {
	...
	exports.default = class ParseCLP {
		...
		*/
		constructor(userId) {
			...
			if (userId && typeof userId === "object") if (userId instanceof _ParseUser.default) {
				...
				if (isValidGroupPermission) if ((0, _every.default)(users).call(users, (pointer) => typeof pointer === "string")) {
		...
		*/
		...
		_setAccess(permission, userId, allowed) {
			...
			if (typeof allowed !== "boolean") throw new TypeError("allowed must be either true or false.");
			if (!this.permissionsMap[permission][userId]) if (!allowed) return;
	...
	var ParseSchema = class {
		...
		*/
		constructor(className) {
			...
			if (typeof className === "string") if (className === "User" && _CoreManager.default.get("PERFORM_USER_REWRITE")) this.className = "_User";
